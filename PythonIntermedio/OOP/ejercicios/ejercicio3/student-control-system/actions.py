...
def student_exists(name, section, students_list):
    ...
    return any(student.matches(name, section) for student in students_list)

def request_student_name():
    """Keeps asking until the user enters a non-empty name."""
    return request_non_empty_text("Ingrese el nombre del estudiante: ")
...
def request_student_info():
    ...
    grades = {
        key: request_for_number(f"Digite la nota de {label}: ", 0, 100)
        for key, label in data.SUBJECT_LABELS.items()
    }
    return std(name, section, grades)


def save_student(students_list):
    """Registers a new Student in the in-memory list, unless one with the same name and section already exists."""
...
def get_grades_averages(students_list):
    ...
    subject_keys = list(data.SUBJECT_LABELS.keys())
    return [{"name": student.name, "average": student.average(subject_keys)} for student in students_list]
...
def delete_student(student_name, student_section, students_list):
    ...
    students_list[:] = [s for s in students_list if not s.matches(student_name, student_section)]
...
def get_reproved_students(students_list):
    ...
    for student in students_list:
        failed_subjects = student.failed_subjects(data.MIN_GRADE, data.SUBJECT_LABELS)
        if failed_subjects:
    ...
    print_numbered_list(
        reproved,
        lambda s: f"{s['name']} - Sección {s['section']} -> " + ", ".join(f"{label}: {grade}" for label, grade in s['failed'])
    )
...
def export_CSV_student_list(students_list):
    ...
    if not file_path:
        print("[info] => No se seleccionó ninguna ruta para exportar.")
        return
